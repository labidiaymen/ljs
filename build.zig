...
pub fn build(b: *std.Build) void {
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            // root module.
            .imports = &.{
                // Here "ljs" is the name you will use in your source code to
                // import this module (e.g. `@import("ljs")`). The name is
    ...
    // ── Code-quality steps (constitution v1.1.0 merge gate) ──────────────────
    // `zig build fmt`        — apply `zig fmt` to the sources
    // `zig build fmt-check`  — verify formatting (CI / pre-merge gate)
    // `zig build lint`       — fmt-check + ZLint (https://github.com/DonIsaac/zlint) if installed
    const fmt_targets = [_][]const u8{ "src", "build.zig" };

    const fmt = b.addSystemCommand(&[_][]const u8{ "zig", "fmt" });
    fmt.addArgs(&fmt_targets);
    const fmt_step = b.step("fmt", "Format sources with `zig fmt`");
    fmt_step.dependOn(&fmt.step);

    const fmt_check = b.addSystemCommand(&[_][]const u8{ "zig", "fmt", "--check" });
    fmt_check.addArgs(&fmt_targets);
    const fmt_check_step = b.step("fmt-check", "Verify formatting with `zig fmt --check`");
    fmt_check_step.dependOn(&fmt_check.step);

    const lint = b.addSystemCommand(&[_][]const u8{ "sh", "scripts/lint.sh" });
